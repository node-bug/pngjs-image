...
var BufferedStream = function (data, offset, length) {
	...
	if (data) {
		if (offset === false) {
			...
			this._data = Buffer.alloc((length || (data.length - (offset || 0))) * 2);
		...
		if (length === 0) {
			...
			this._data = Buffer.alloc(length || 1024 * 500);
		}
	}
};
...
BufferedStream.prototype._resize = function () {
	var buffer = Buffer.alloc(this._data.length * 2);
...
BufferedStream.prototype.writeString = function (text, encoding) {
	this.writeBuffer(Buffer.alloc(text, encoding || 'utf8'));
};
